...
#define DRIVER_NAME "lcd_original"
#define DRIVER_CLASS "LCDjModuleClass"
...
void lcd_send_byte(char data) {
	printk("Entered lcd_send_byte\n");
	int i;
	for(i=0; i<8; i++)
		gpio_set_value(gpios[i+2], (((data) & (1<<i)) >> i) & 0x01);
	lcd_enable();
	msleep(5);
}
...
static ssize_t driver_write(struct file *File, const char *user_buffer, size_t count, loff_t *offs) {
	...
	printk("Entered write in driver\n");
...
static int __init ModuleInit(void) {
	...
	lcd_command(0x0e);	/* Turn display on, turn cursor on, set cursor blinking */
	msleep(10);
