 ...
 */
void lcd_send_byte(char data, int inc) {
	int i;
	for(i=0; i<4; i++)
		//gpio_set_value(gpios[i+2], (((data) & (1<<i)) >> i) & 0x1);
		gpio_set_value(gpios[i+2], (((data) & (1<<(i + inc))) >> (i + inc)) & 0x1);
     msleep(1000);
	lcd_enable();
	msleep(5);
}
 ...
 *
 * @Working: You provide the command as if you are writing to the Lower 4 bits. For this reason I am using 0 as the second argument to the function lcd_send_byte
 *
 */
 ...
 * @param data: command to send
 *
 * @Working : You provide the data to the function. The functions picks up the 1 byte or 8 bits, and first writes the Upper 4 bits to the Data Pins, and pulses enable to latch. Then the function picks up the Lower 4 bits and writes them to the Data Pins, and pulses enable to latch the data.
 */
...
static int __init ModuleInit(void) {
	...
	lcd_command(0x1);
     ...
     lcd_command(0xc);//Function set N=1 F=1
