...
static int hc05_recv(struct serdev_device *serdev, const unsigned char *buffer, size_t size) {
	...
	printk("hc05 - Received %ld bytes with %s", size, buffer);
     return 1;
}
...
static int hc05_probe(struct serdev_device *serdev) {
	int status;
    const char *at_cmd = "AT+UART=9600,0,0\r\n";
    ...
    serdev_device_write_buf(serdev, at_cmd, strlen(at_cmd));
...
static int __init my_init(void) {
	...
	if(serdev_device_driver_register(&hc05_driver) != 0) {
		printk("hc05 - Error! Could not load driver\n");
		return -1;
	}
     printk("Reached end of my_init here\n");
	return 0;
}
...
module_exit(my_exit);
#endif
module_serdev_device_driver(hc05_driver);
